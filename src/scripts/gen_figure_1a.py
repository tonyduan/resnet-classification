...
if __name__ == "__main__":
    ...
    EXPERIMENTS = ("cifar100_wd_1e-5_mixup_0.0",)
    ...
    for experiment_name in EXPERIMENTS:
        ...
        for split, interval, key in itertools.product(("train", "test", "posthoc"), ("lower", "upper"),
            ...
            if split != "posthoc":
                ...
                df["eval"].extend(["default"] * len(experiment_df))
                ...
                col = f"{interval}_{key}"

                df["experiment"].extend(["_".join(experiment_name.split("_")[1:])] * len(experiment_df))
                df["eval"].extend(["posthoc"] * len(experiment_df))
                df["interval"].extend([interval] * len(experiment_df))
                df["split"].extend(["test"] * len(experiment_df))
                df["key"].extend([key] * len(experiment_df))
                df["value"].extend([eval_df[col].iloc[0]] * len(experiment_df))
                df["epoch"].extend(np.arange(len(experiment_df)) + 1)
    ...
    grid = sns.relplot(data=df, kind="line", x="epoch", y="value", row="experiment", col="key",
                       ...
                       size=1,
                       style="eval",
                       dashes={"default": "", "posthoc": (2, 5)},
                       palette=sns.color_palette("gray", 2),
                       col_order=("nll", "acc", "toplabel_ece", "consistency_toplabel_ece"),
                       aspect=1.2, height=1.6, facet_kws={"sharex": True, "sharey": False, "legend_out": True})
